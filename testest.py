...
if uploaded_file:
    try:
        ...
        if "x" not in df.columns or "y" not in df.columns:
            st.error("O arquivo Excel deve conter as colunas 'x' (longitude) e 'y' (latitude).")
        else:
            # Verificar se a coluna "Estação" está presente
            if "Estação" not in df.columns:
                st.error("O arquivo Excel deve conter uma coluna chamada 'Estação'.")
            else:
                # Filtro para selecionar a estação de monitoramento
                estacoes = df["Estação"].unique()
                ...
                df_filtrado = df[df["Estação"].isin(estacao_selecionada)]
